...
export const PhysicsBall = ({
  ...
  const addPoints = (
    ...
    if (
      pointsToAdd >= 20 &&
      now - lastScreenShakeTime.current > screenShakeThrottle
    ) {
      setScreenShake(true);
  ...
  const handleDrag = (
    ...
    wiggleIntensity.current = Math.min(
      1,
      Math.max(0, Math.abs(info.velocity.x) / 1000),
    );
    ...
    if (dragDistance.current > 50) {
      const points = Math.min(
        10,
        Math.max(3, Math.floor(dragDistance.current / 20)),
      );
    ...
    const updatePhysics = () => {
      ...
      if (newY >= ballBottom) {
        ...
        if (now - lastBounceTime.current > 300) {
          ...
          const bouncePoints = Math.min(
            30,
            Math.max(10, Math.floor(bounceForce / 100)),
          );
      ...
      if (
        Math.abs(newY - lastY) < 0.1 &&
        Math.abs(velocityY.current) < 10 &&
        newY >= ballBottom - 1
      ) {
        velocityY.current = 0;
        newY = ballBottom;
  ...
  return (
    <div
      ...
      className={`relative bg-gradient-to-b from-gray-800 to-gray-900 overflow-hidden ${
        fullScreen
          ? "w-screen h-screen"
          : "w-full h-[500px] rounded-lg border border-gray-700"
      }`}
      style={{
        touchAction: "none",
        ...getScreenShakeStyle(),
      ...
      <div className="absolute top-0 right-0 p-3 z-20 pointer-events-none">
        <div className="bg-black/50 backdrop-blur-sm px-6 py-3 rounded-lg shadow-lg border border-gray-700">
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              className={`text-2xl font-bold ${multiplier > 1 ? "text-yellow-400" : "text-gray-500"}`}
            >
              {multiplier}x
            </div>
          </div>
          ...
          <div
            ...
            style={{ color: combo >= 100 ? "#FFFFFF" : glizzyColor }}
          >
            {getColorName(combo)}
          </div>
        </div>
                        ...
                        {highScores.map((entry, index) => (
                          <tr
                            ...
                            className={`border-b border-gray-800 ${
                              entry.score === score && isNewHighScore
                                ? "bg-yellow-900/30"
                                : ""
                            }`}
                          >
          ...
          style={{
            ...
            boxShadow: isOnFire
              ...
              : multiplier > 1
                ...
                : "none",
            transition:
              "background-color 0.3s, background 0.3s, box-shadow 0.3s",
          }}
      ...
      {!gameActive && !showResults && (
        <div className="absolute top-32 left-0 right-0 flex justify-center z-40 pointer-events-none">
          <div className="bg-black/70 backdrop-blur-sm px-6 py-4 rounded-xl text-white text-center max-w-md mx-4 border border-gray-700 shadow-lg">
            ...
            <p className="mb-4">
              Drag the glizzy to score points. You have 10 seconds!
            </p>
      ...
      {gameActive && (
        <div className="absolute bottom-4 left-0 right-0 flex justify-center pointer-events-none">
          <div className="bg-black/50 backdrop-blur-sm px-4 py-2 rounded-full text-white text-sm">
            Wiggle faster for more points!{" "}
            <Zap className="inline-block ml-1 text-yellow-400" size={16} />
          </div>
        </div>
      )}
    </div>
